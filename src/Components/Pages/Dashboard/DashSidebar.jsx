...
const DashSidebar = () => {
  ...
  return (
    <div className="dash-active min-h-screen max-h-screen sticky top-0 left-0 bottom-0 z-30 flex">
      ...
      >
        <div className={`flex items-center border-b border-base-300 gap-2 py-3 bg-base-300 ${collapsed ? "justify-center px-2" : "px-3"}`}>
          <a className={`flex items-center gap-2 ${collapsed ? "justify-center" : ""}`} href="/">
            <IoSchool className="text-green-600 text-xl" />
            <span className={`${collapsed ? "hidden" : "inline"} font-bold text-lg truncate`}>
        ...
        <nav className="flex-1 overflow-auto">
          <ul className={`menu p-0 ${collapsed ? "w-full" : "w-50"}`}>
            <li>
              <Link
                to=""
                className={`flex items-center gap-2 ${collapsed ? " justify-center" : "pl-4"} min-w-0`}
              >
                <IoHomeOutline className="text-xl" />
                <span className={`${collapsed ? "hidden" : "inline"} truncate min-w-0`}>Dashboard</span>
              </Link>
            </li>
            ...
            <li>
              ...
              >
                ...
                <span className={`${collapsed ? "hidden" : "inline"} truncate min-w-0`}>My Profile</span>
              </NavLink>
            </li>
